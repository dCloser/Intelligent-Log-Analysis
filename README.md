...
1.  **pfSense/OPNsense** → syslog → **Wazuh manager**
2.  **Wazuh** → Filebeat → **OpenSearch/Elasticsearch indices** (`wazuh-archives-*`, `wazuh-alerts-*`)
3.  **Wazuh Dashboard** (OpenSearch Dashboards) → **Alerting** (Query-level Monitor)
4.  **Alerting Action** → **Notifications Channel** (Custom Webhook) → **FastAPI receiver** (`/wazuh`)
5.  **FastAPI** → JSONL + SQLite → `/ui` (review + interpretation)
6.  *Optional:* **FastAPI** → **Ollama** (local LLM) → per-event interpretation
...
```bash
curl -k -u 'admin:<REDACTED>' '[https://127.0.0.1:9200/_cat/indices/wazuh*?v](https://127.0.0.1:9200/_cat/indices/wazuh*?v)'
```
...
# Notes:
  ...
  The key piece is _"ctx.results.0.hits.hits"_, which contains the actual documents. If you don’t reference it, you’ll only see monitor metadata.
...
Environment=OLLAMA_URL=[http://127.0.0.1:11434/api/generate](http://127.0.0.1:11434/api/generate)
